...
import { FolderOpen, MessageSquare, Upload, Lightbulb, Send, File, Clock, CheckCircle2, AlertCircle, Loader2, ExternalLink, Globe, ArrowRight, Sparkles, Link2 } from 'lucide-react';
interface Project {
  id: string;
  project_code: string;
  user_id: string | null;
...
const Dashboard = () => {
  ...
  const uploadResource = async () => {
    ...
    try {
      ...
      const {
        error: uploadError
      } = await supabase.storage.from('project-resources').upload(fileName, resourceFile);
                      ...
                      <CardContent>
                        <ScrollArea className="h-[400px] pr-4 mb-4">
                          ...
                          delay: index * 0.05
                                  ...
                                  <div className={`max-w-[80%] p-4 rounded-2xl ${msg.is_admin ? 'bg-muted text-foreground rounded-bl-md' : 'bg-gradient-to-r from-primary to-primary/80 text-primary-foreground rounded-br-md shadow-lg shadow-primary/20'}`}>
                                    <p className="text-sm">{msg.message}</p>
                                    <p className={`text-xs mt-2 ${msg.is_admin ? 'text-muted-foreground' : 'text-primary-foreground/70'}`}>
                                      {msg.is_admin ? '👨‍💻 Telvix Team' : 'You'} • {new Date(msg.created_at).toLocaleTimeString([], {
                                hour: '2-digit',
                                minute: '2-digit'
                              })}
                                    </p>
                        ...
                        <div className="flex gap-2">
                          <Input placeholder="Type your message..." value={newMessage} onChange={e => setNewMessage(e.target.value)} onKeyDown={e => e.key === 'Enter' && !e.shiftKey && sendMessage()} className="flex-1 rounded-xl bg-background" />
                    ...
                    <Card className="backdrop-blur-sm bg-card/80">
                      ...
                      <CardContent className="space-y-6">
                        <div className="border-2 border-dashed border-primary/30 rounded-2xl p-6 bg-primary/5">
                          <div className="space-y-4">
                            <div className="space-y-2">
                              ...
                              <Input id="resourceFile" type="file" onChange={e => setResourceFile(e.target.files?.[0] || null)} className="file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-primary file:text-primary-foreground hover:file:bg-primary/80" />
                            </div>
                            <div className="space-y-2">
                              <Label htmlFor="resourceDesc">Description (Optional)</Label>
                              <Input id="resourceDesc" placeholder="Brief description of this file" value={resourceDesc} onChange={e => setResourceDesc(e.target.value)} className="rounded-xl bg-background" />
                            </div>
                        ...
                        <div className="space-y-3">
                          ...
                          {resources.length === 0 ? <div className="text-center py-12 text-muted-foreground">
                              <File className="w-12 h-12 mx-auto mb-4 opacity-50" />
                              <p>No files uploaded yet</p>
                            </div> : resources.map((resource, index) => <motion.div key={resource.id} initial={{
                        opacity: 0,
                        y: 10
                      }} animate={{
                        opacity: 1,
