...
const Footer = () => {
  ...
  const handleNewsletterSubmit = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      await emailjs.send('service_nzx6w0k', 'template_krkmosf', {
          ...
          <div className="grid lg:grid-cols-2 gap-16 mb-16">
            ...
            <div>
              ...
              <form onSubmit={handleNewsletterSubmit} className="flex gap-3 max-w-md">
                <Input type="email" placeholder="Enter your email" value={email} onChange={e => setEmail(e.target.value)} className="bg-background/10 border-background/20 text-background placeholder:text-background/40 rounded-full px-6" required disabled={isLoading} />
                <Button type="submit" className="bg-primary hover:bg-primary/90 rounded-full px-6" disabled={isLoading}>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 py-16 border-t border-background/10">
            ...
            <div>
              ...
              <div className="flex gap-3">
                {socialLinks.map(social => <a key={social.label} href={social.href} className="w-10 h-10 bg-background/10 rounded-full flex items-center justify-center hover:bg-primary transition-colors" aria-label={social.label}>
          ...
          <div className="pt-8 border-t border-background/10">
            <div className="flex flex-col md:flex-row justify-between items-center gap-4">
              <div className="flex items-center gap-2">
                <img src={telvixLogo} alt="Telvix" className="h-16 md:h-20 w-auto brightness-0 invert" />
              </div>
