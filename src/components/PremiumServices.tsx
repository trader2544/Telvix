...
const PremiumServices = () => {
  const premiumServices = [{
    ...
    price: "KSh 3,500",
    features: ["Personalized project assessment", "Technology recommendations", "Roadmap planning", "Q&A session", "Follow-up summary report"],
    popular: true
  }, {
    icon: FileText,
    ...
    price: "KSh 7,000",
    features: ["Code review and analysis", "Performance assessment", "Security evaluation", "Improvement roadmap", "Technical documentation"]
  }];
  const digitalResources = [{
    ...
    price: "KSh 2,500",
    features: ["20+ pre-built workflows", "Email automation templates", "CRM integration workflows", "Social media automation", "E-commerce automations", "Setup tutorials"]
  }, {
    icon: FileText,
    title: "Web Development Starter Kit",
    description: "Complete templates and guides for launching your web project.",
    price: "KSh 4,000",
    features: ["5 responsive website templates", "SEO optimization guide", "Performance checklist", "Deployment tutorials", "Maintenance guidelines"]
  }];
  ...
  return <div className="space-y-8">
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
          ...
          return <Card key={index} className={`relative bg-white/90 backdrop-blur-sm border-0 shadow-lg ${service.popular ? 'ring-2 ring-accent' : ''}`}>
                ...
                <CardContent className="space-y-4 pt-0">
                  ...
                  <ul className="space-y-1.5">
                    {service.features.map((feature, featureIndex) => <li key={featureIndex} className="flex items-start gap-2 text-xs md:text-sm">
                        <div className="w-1.5 h-1.5 bg-green-500 rounded-full mt-2 flex-shrink-0"></div>
                        <span>{feature}</span>
                      </li>)}
                  </ul>
                  <Button onClick={() => handleBookConsultation(service.title)} className="w-full bg-gradient-to-r from-primary to-accent hover:from-primary/90 hover:to-accent/90 text-sm">
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
          ...
          return <Card key={index} className="bg-white/90 backdrop-blur-sm border-0 shadow-lg">
                <CardHeader className="pb-3">
                  <CardTitle className="flex items-center gap-2 text-lg">
                ...
                <CardContent className="space-y-4 pt-0">
                  ...
                  <ul className="space-y-1.5">
                    {resource.features.map((feature, featureIndex) => <li key={featureIndex} className="flex items-start gap-2 text-xs md:text-sm">
                        <div className="w-1.5 h-1.5 bg-blue-500 rounded-full mt-2 flex-shrink-0"></div>
                        <span>{feature}</span>
                      </li>)}
                  </ul>
                  <Button onClick={() => handlePurchaseResource(resource.title)} variant="outline" className="w-full border-primary text-primary hover:bg-primary hover:text-white text-sm">
